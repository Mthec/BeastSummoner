...
public class BeastSummonerEditTagsQuestion extends BeastSummonerQuestionExtension {
    private final List<String> allTags;

    BeastSummonerEditTagsQuestion(Creature responder) {
    ...
    public void answer(Properties properties) {
        ...
        if (renameTags || removeTags) {
            ...
            if (renameTags) {
                ...
                new BeastSummonerRenameTagsQuestion(getResponder(), selected).sendQuestion();
            } else {
                int removed = 0;
                for (String tag : selected) {
                    try {
                        BeastSummonerMod.mod.db.deleteTag(tag);
                        ++removed;
    ...
    public void sendQuestion() {
        ...
        String bml = new BMLBuilder(id)
                             ...
                             .newLine()
                             .text("Beast summoners with removed tags will be reverted to unique inventory and tag inventory will be deleted.  This cannot be undone.")
                             .newLine()
