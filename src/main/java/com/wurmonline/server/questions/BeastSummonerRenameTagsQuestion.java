...
public class BeastSummonerRenameTagsQuestion extends BeastSummonerQuestionExtension {
    private final List<String> toRename;

    BeastSummonerRenameTagsQuestion(Creature responder, List<String> toRename) {
        ...
        if (wasSelected("rename")) {
            ...
            for (String item : properties.stringPropertyNames()) {
                if (item.startsWith("t")) {
                    ...
                    try {
                        BeastSummonerMod.mod.db.renameTag(toRename.get(i), properties.getProperty(item));
                        ++renamed;
    ...
    public void sendQuestion() {
        ...
        String bml = new BMLBuilder(id)
                             .table(new String[] { "From", "To" }, toRename,
                                     (tag, b) -> b.label(tag).entry("t" + rowNumber.getAndIncrement(), tag, BeastSummonerMod.maxTagLength))
                             .newLine()
